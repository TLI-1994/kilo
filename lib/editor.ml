...
let create () =
  ...
  Some
    {
      screenrows = rows - 2;
      (* -2 to make room for status bar and message *)
      screencols = cols;
      cx = 0;
      cy = 0;
      rowoff = 0;
      ...
      quitting_count = 0;
      search_context =
        { direction = Forward; query = ""; matching = "" };
    }
...
let open_file t filename =
  let buf =
    BatFile.with_file_in filename (fun input ->
        ...
        try
          readline input 0
            (Editor_buffer.create (BatIO.read_line input))
...
let welcome_string width =
  let welcome =
    sprintf "Kilo editor -- version %s" Settings.kilo_version
  in
  let padding =
    String.make (((width - String.length welcome) / 2) - 1) ' '
  in
  "~" ^ padding ^ welcome
...
let draw_rows t =
  for y = 0 to t.screenrows - 1 do
    ...
    let row =
      (* text buffer *)
      if filerow < rows t.buf then
        begin
          Editor_buffer.get_sub t.buf ~y:filerow ~x:t.coloff
            ~len:t.screencols
          |> Highlight.highlight ~matching:t.search_context.matching
               t.syntax
             (* welcome text *)
        end
      else if rows t.buf = 1 && cols t.buf 0 = 0 && y = t.screenrows / 3
      then welcome_string t.screencols (* out of buffer *)
      else "~"
    in
    write row;
...
let fit ?(pad = ' ') len ~left ~right =
  ...
  if left_len + right_len <= len then
    ...
    left ^ padding ^ right
  else StringLabels.sub left ~pos:0 ~len:(len - right_len) ^ right

let draw_status_bar t =
  let bar =
    fit t.screencols
      ~left:
        (sprintf "%s%s - %d lines - %d cols"
           (BatOption.default "[No Name]" t.filename)
           (if t.dirty then " [+]" else "")
           (rows t.buf) (cols t.buf t.cy))
      ~right:
        (sprintf "%s | %d/%d" t.syntax.filetype (t.cy + 1) (rows t.buf))
  in
...
let draw_message_bar t =
  ...
  (* show 5 seconds *)
  write
  @@ if Unix.time () -. t.statusmsg_time < 5. then t.statusmsg else ""
...
let prompt ?(callback = fun _ -> ()) t msgfmt =
  let rec prompt' t input =
    ...
    | Esc ->
        set_statusmsg t "";
        None
    | Backspace -> prompt' t (BatString.rchop ~n:1 input)
    | Ctrl 'H' -> prompt' t (BatString.rchop ~n:1 input)
    | Ch c -> prompt' t (sprintf "%s%c" input c)
    | _ -> prompt' t input
  in
  prompt' t ""
...
let find t =
  ...
  match
    prompt t "Search: %s (ESC to cancel)"
      ~callback:(search_and_jump t t.cy)
  with
  | None ->
      (* reset position *)
...
let find_next t =
  t.search_context.direction <- Forward;
  let start_y =
    if t.cy >= rows t.buf - 1 then rows t.buf - 1 else t.cy + 1
  in
...
let get_filename t =
  if Option.is_some t.filename then t.filename
  else
...
let save_file t =
  ...
  | None -> set_statusmsg t "Save aborted"
  | Some filename ->
      let p = perm [ user_read; user_write; group_read; other_read ] in
      with_file_out ~mode:[ `create; `trunc ] ~perm:p filename
        (fun output ->
  ...
  let result =
    ...
    | Ctrl 'Q' ->
        if t.dirty && t.quitting_count < 1 then begin
          ...
          set_statusmsg t
            "WARNING!!! File has unsaved changes. Press Ctrl-Q again \
             to quit.";
          `Wait
        end
        else begin
    ...
    | Arrow_left ->
        ...
        `Continue
    | Page_up ->
        move_cursor t `Full_up;
        `Continue
    | Page_down ->
        move_cursor t `Full_down;
        `Continue
    | Home ->
        move_cursor t `Head;
        `Continue
