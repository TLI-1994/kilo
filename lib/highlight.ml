...
module Syntax = struct
  ...
  let default = { filetype = "no filetype"; filematch = []; rule = [] }
  ...
  let detect_filetype filename =
    ...
    match
      List.filter (fun syntax -> List.mem extension syntax.filematch) db
    with
    | [] -> default
    | hd :: _ -> hd
end
...
let find_at pos patterns text =
  let rec loop patterns =
    ...
    | pattern :: tl -> (
        try
          let found =
            Pcre.pcre_exec ~pos ~pat:pattern.regex ~flags:[ `NOTEMPTY ]
              text
          in
          let found_start, found_end = (found.(0), found.(1)) in
          if found_start <> pos then loop tl
          else (pattern.group, found_end - found_start)
...
let%test_module "tests" =
  (module struct
    ...
    let%test _ =
      find_at 0 ocaml_syntax.rule "\"foo\" 12.32" = (String, 5)

    let%test _ =
      find_at 6 ocaml_syntax.rule "\"foo\" 12.32" = (Number, 5)
...
let get_highlights ~matching patterns text =
  ...
  let rec loop i hlgroups =
    if i >= max then List.rev hlgroups
    else
...
let colorize text hlgroups =
  ...
  colored_text :=
    !colored_text ^ Terminal.Escape_command.color Terminal.Default;
  !colored_text
